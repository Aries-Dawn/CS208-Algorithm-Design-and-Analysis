...
public class Evaluator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(expect-->0) {
            generateInputData();
            try {
                runJava(FaFaTa.class, input, 0);
                runCpp("cpps\\labB-2.exe", input, 1);
...
//        System.out.println("Result: "+res[0]);
    ...
    private static void runCpp(String exe,StringBuilder input,int index) throws Exception {
        ...
        String[] cmd = { "cmd.exe", "/c", " cpps\\labB-2.exe <in.txt >result.txt" };
    ...
    private static void generateInputData() {
        ...
        input.append(n).append(" ").append(primes[r.nextInt(primes.length)]).append("\n");
        for (int i = 0; i < n; i++) {
